...
namespace movies.api.Controllers
{
    ...
    public class PeopleController : ControllerBase
    {
        ...
        public PeopleController(IOptions<MovieDBInfo> movieInfo, IUserSearchLogService userSearchLogService, IHttpClientService httpClientService)
        {
        ...
        public async Task<string> SearchById(int id, [FromBody] DetailSearchParameter searchParameter)
        {
            try
            {
                ...
                var url = GenerateUrlForPeopleDetail(id, searchParameter);
        ...
        private string GenerateUrl(SearchParameter searchParameter)
        {
            ...
            url = _movieDBInfo.PersonApiAccessPoint + $"?api_key={_movieDBInfo.Api_Key}&language={searchParameter.Language}&page={searchParameter.Page}&include_adult={searchParameter.Include_Adult}&query={searchParameter.Query}";
            return url;
        }

        private string GenerateUrlForPeopleDetail(int peopleId, DetailSearchParameter searchParameter)
        {
            var url = string.Empty;
            if (searchParameter.Language == string.Empty)
                searchParameter.Language = "en-US";

            url = _movieDBInfo.PersonDetailApiAccessPoint + peopleId + $"?api_key={_movieDBInfo.Api_Key}&language={searchParameter.Language}";
            return url;
        }
    }
}
